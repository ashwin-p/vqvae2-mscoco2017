    ...
    def forward(self, input, condition=None, cache=None):
        ...
        if condition is not None:
            if 'condition' in cache:
                ...
                # If the condition is continuous (floating point), use it directly.
                ...
                # Only apply cond_resnet if the input has the expected number of channels.
                if hasattr(self, 'cond_resnet') and condition_feature.shape[1] == self.n_class:
                    condition_feature = self.cond_resnet(condition_feature)
                    condition_feature = F.interpolate(condition_feature, scale_factor=2)
                cache['condition'] = condition_feature.detach().clone()
                condition = condition_feature[:, :, :height, :]
